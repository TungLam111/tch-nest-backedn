...
export class ProductController {
    ...
    @Get('search')
    async searchProductByName(@Res() res: any, @Query() searchQuery: SearchProductDto) {
    ...
    @Put()
    async updateProduct(@Res() res: any, @Body() product: UpdateProductDto) {
        const result = await this.productService.updateProduct(product)
        res.status(result.status).json(result.content);
    }

    @Post('add-to-menu')
    async addProductToMenu(@Res() res: any, @Body() product: AddProductToMenuDto) {
